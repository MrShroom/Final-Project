...
public class OctreeNode {
	...
	public void Add(GameObject gameObject){
		...
		//else create the children and push down the objects to the next level
			...
			if (objectBounds.center.Equals (bounds.center) ||
				objects.Count > 0 && objects.TrueForAll (i => objectBounds.center.Equals (i.GetComponent<Bounds> ().center)))
				objects.Add (gameObject);
	...
	private void PushObjectsDown(){
		foreach (GameObject o in objects) {
			Bounds objectBounds = o.GetComponent<Bounds>();
			if(!objectBounds.center.Equals (bounds.center)) {
				AddToChildren(o);
				objects.Remove(o);
			}
	...
	private void AddToChildren(GameObject gameObject){
		foreach (OctreeNode c in children)
			if (c.bounds.Intersects (gameObject.GetComponent<Bounds> ()))
				c.Add (gameObject);
	}
